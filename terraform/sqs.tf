...
resource "aws_sqs_queue_policy" "terminate_instance_queue_policy" {
  queue_url = aws_sqs_queue.terminate_instance_queue.id
  ...
  "Statement": [
    {
      ...
      "Principal": "*",
      "Action": "sqs:SendMessage",
      "Resource": "${aws_sqs_queue.terminate_instance_queue.arn}",
      "Condition": {
        "ArnEquals": {
          "aws:SourceArn": "${aws_cloudwatch_event_rule.instance_id.arn}"
        }
...
resource "aws_sqs_queue_policy" "terminate_instance_dl_queue_policy" {
  queue_url = aws_sqs_queue.terminate_instance_dl_queue.id
  ...
  "Statement": [
    {
      ...
      "Principal": "*",
      "Action": "sqs:SendMessage",
      "Resource": "${aws_sqs_queue.terminate_instance_dl_queue.arn}",
      "Condition": {
        "ArnEquals": {
          "aws:SourceArn": "${aws_cloudwatch_event_rule.instance_id.arn}"
        }
